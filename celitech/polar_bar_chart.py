...
def select_data_on_day():
    # Determine DAY based off CONNECT_TIME
...
def select_dur_on_month():
    ...
    for index, row in df.iterrows():
        connect_time = str(row["CONNECT_TIME"])
        month = int(connect_time[5:7])
        num_calls[month] += int(row["DURATION"]) / (1E6) # Bytes to MB
...
def select_data_on_timesplit(datatype, time):
    ...
    for index, row in df.iterrows():
        connect_time = str(row["CONNECT_TIME"])
        local_day = m_day = int(connect_time[8:10])
        month = int(connect_time[5:7])
